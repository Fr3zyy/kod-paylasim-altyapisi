...
app.post("/sharing", async (req, res) => {
  ...
  if (cooldown.get(req.user.id) && cooldown.get(req.user.id).count >= 10) return error(res, 429, "10 dakika içerisinde en fazla 3 kod paylaşabilirsin!");
...
app.get("/normal/:codeID", async (req, res) => {
  ...
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (member && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen discord sunucumuzdaki bilgilendirme kanalını okuyunuz!");
...
app.get("/gold/:codeID", async (req, res) => {
  ...
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.goldRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen discord sunucumuzdaki bilgilendirme kanalını okuyunuz!");
  const codeData = require("./src/schemas/code");
...
app.get("/diamond/:codeID", async (req, res) => {
  ...
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.diaRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen discord sunucumuzdaki bilgilendirme kanalını okuyunuz!");
  const codeData = require("./src/schemas/code");
...
app.get("/ready/:codeID", async (req, res) => {
  ...
  const codeID = req.params.codeID;
  if (!codeID) return res.redirect("/");
  if (member && !member.roles.cache.has(conf.readySystemsRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen discord sunucumuzdaki bilgilendirme kanalını okuyunuz!");
  const codeData = require("./src/schemas/code");
...
app.use((req, res) => error(res, 404, "Sayfa bulunamadı Daha Sonra Tekrar Deneyiniz !"));
